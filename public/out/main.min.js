...
(() => {
  ...
  function n(n) {
    null !== n && t.spinEl.classList.add("visually-hidden");
    ...
    ((o.innerHTML = ""),
      a.forEach((t, n) => {
        ...
        (l.classList.add("main__span"),
          a.classList.add("main__item"),
          i.classList.add("main__img"),
          r.classList.add("main__link"),
          (i.src = t.album.images[1]?.url || t.album.images[0]?.url || ""),
          (i.alt = t.album.name),
          (r.href = t.external_urls.spotify),
          (r.target = "_blank"),
          (r.rel = "noopener noreferrer nofollow"),
          (r.textContent = `${t.name} - ${t.artists[0].name}`),
          (l.textContent = "click"),
          o.appendChild(a),
          r.appendChild(l),
          a.appendChild(i),
  ...
  (o.length > 0 &&
    ...
    t.formEl.addEventListener("submit", async (e) => {
      ...
      if (!a)
        return (
          (t.inputEl.placeholder = "[error]"),
          void console.error("[error]")
        );
      (t.spinBackEl.classList.remove("visually-hidden"),
        t.spinEl.classList.remove("visually-hidden"),
        (t.inputEl.placeholder = "Type to search..."));
      try {
        (localStorage.clear(),
        ...
        (t.spinBackEl.classList.add("visually-hidden"),
          t.spinEl.classList.add("visually-hidden"));
      }
      t.formEl.reset();
    }),
                  ...
                  (async function (e) {
                    try {
                      const t = await fetch(`/auth/twitch/callback?code=${e}`, {
                        ...
                        { login: n, code: o } = await t.json();
                      n &&
                        (localStorage.setItem("_code", o),
                        localStorage.setItem("_login", n),
                        a());
                    } catch (e) {
                      console.error(e);
                    }
