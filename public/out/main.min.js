...
!function () {
  ...
  console.clear(), e.formEl.addEventListener("submit", t => {
    ...
    localStorage.setItem("_input", n), localStorage.getItem("_input"), n ? (async function (t) {
      await fetch("/", {
        ...
        o.innerHTML = "", n.forEach((e, t) => {
          ...
          l.classList.add("main__span"), n.classList.add("main__item"), i.classList.add("main__img"), r.classList.add("main__link"), i.src = e.album.images[1].url, i.alt = e.album.name, r.href = e.external_urls.spotify, r.target = "_blank", r.rel = "noopener noreferrer nofollow", r.textContent = "".concat(e.name, " - ").concat(e.artists[0].name), l.textContent = "click", o.appendChild(n), r.appendChild(l), n.appendChild(i), n.appendChild(r);
        });
  ...
  }), e.loginBtn.addEventListener("click", async function () {
    const t = window.open("/auth/twitch"),
      n = await new Promise(n => {
        const o = l => {
          if (l.origin === window.location.origin) {
            const i = l.data;
            console.log(i), !0 === i.login ? e.loginBtn.textContent = "Logout" : console.error("error"), t.close(), window.removeEventListener("message", o), n(l);
          }
        };
    ...
    !0 === o.login ? e.loginBtn.textContent = "Logout" : console.error("error"), t.close();
  });
}();