...
!(function () {
	...
	const t = JSON.parse('{"a":["After Dark","Freaks","love"]}').a,
		a = btoa('74e32fcef00f4917950e482de83f327d:7d7aece049984959b34744653fcb3d1b');
	let n = 1;
	console.clear(),
		e.formEl.addEventListener('submit', o => {
			o.preventDefault();
			const l = e.inputEl.value.trim();
			console.log(l),
				localStorage.setItem('input', l),
				localStorage.getItem('input'),
				'' === l
					? ((e.inputEl.placeholder = '[error]'), console.error('[error]'))
					: ((async function (o) {
							const l = await fetch('https://accounts.spotify.com/api/token', {
									method: 'POST',
									headers: {
								...
								r = (await l.json()).access_token;
							try {
								const a = await fetch('https://api.spotify.com/v1/search?type=track&q='.concat(o, '&limit=15'), {
										headers: {
								...
								(i.innerHTML = ''),
									l.tracks.items.forEach(e => {
										...
										JSON.stringify(localStorage.setItem(s, c)),
											n++,
											...
											(l.onload = () => {
												t.includes(e.name) &&
													...
													(r.style.padding = '30px'),
													(r.style.fontWeight = '500'));
											}),
											(l.src = e.album.images[1].url),
											(l.alt = e.album.name),
											(r.href = e.external_urls.spotify),
											(r.target = '_blank'),
											(r.rel = 'noopener noreferrer nofollow'),
											(r.textContent = ''.concat(e.name, ' - ').concat(e.artists[0].name)),
											(o.textContent = 'click'),
											i.appendChild(a),
											r.appendChild(o),
											a.appendChild(l),
